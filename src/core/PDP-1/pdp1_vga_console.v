...
parameter
   ...
   switch                        = 1'b1;
	...
	localparam	VID_V_BPORCH = 'd15;
	localparam	VID_V_ACTIVE = 'd720;
	localparam	VID_V_TOTAL  = 'd748;
	localparam	VID_H_BPORCH = 'd104;
	localparam	VID_H_ACTIVE = 'd720;
	localparam	VID_H_TOTAL  = 'd928;
...
always @(posedge clk) begin         
   inside_visible_area <= (horizontal_counter >= (VID_H_BPORCH + VID_H_ACTIVE));
      
   current_y <= (vertical_counter >= VID_V_BPORCH && vertical_counter < (VID_V_BPORCH + VID_V_ACTIVE)) ? vertical_counter - VID_V_BPORCH   : 11'b0;
   current_x <= (horizontal_counter >= VID_H_BPORCH && horizontal_counter < (VID_H_BPORCH + VID_H_ACTIVE))    ? horizontal_counter - VID_H_BPORCH : 11'b0; 
   
   if (vertical_counter == VID_V_TOTAL - 1) begin            
